  ...
  return <div className="appShell">
    ...
    <main className="mainArea">
      <header className="topbar">
        ...
        {accountOpen && <div className="popover accountMenu"><div className="accountSummary"><Avatar text={identity.initials}/><div><b>{identity.fullName}</b><span>{identity.groupName} · {identity.levelName}</span></div></div><p>МОЙ АККАУНТ</p><button onClick={()=>go("profile")}><span>♙</span>Личный кабинет<i>→</i></button>{canScan&&<button onClick={()=>window.location.assign("/scan")}><span>⌗</span>QR Scanner<i>→</i></button>}{identity.role==="admin"&&<button onClick={()=>go("admin")}><span>◈</span>Admin Panel<i>→</i></button>}<button className="signOut" onClick={()=>window.location.assign("/signout-with-chatgpt?return_to=/")}>Выйти</button></div>}
      </header>
