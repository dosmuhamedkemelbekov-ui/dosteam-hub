...
export async function GET() {
  ...
  try {
    const [events, clubs, rooms, leaderboard, summary] = await Promise.all([
      ...
      db.prepare("SELECT (SELECT COUNT(*) FROM users WHERE role='student' AND status='active') students,(SELECT COUNT(*) FROM clubs WHERE status='active') clubs,(SELECT COUNT(*) FROM events WHERE status IN ('published','completed')) events,(SELECT COUNT(*) FROM registrations WHERE status!='cancelled') registrations,(SELECT COUNT(*) FROM registrations WHERE status='attended') attended").first(),
    ]);
